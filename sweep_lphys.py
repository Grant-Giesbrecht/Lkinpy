...
lphys_list = np.linspace(0.4, 0.6, 11)

# Create array of bias values
Ibias = np.linspace(1, 33, 34)*1e-3

# cm = CMap('plasma', data=L0_list)
cm = CMap('cividis', data=lphys_list)
results = []
for idx, l_phys in enumerate(lphys_list):
	
	logging.main(f"{Fore.LIGHTBLUE_EX}Beginning simulation of l_phys = {rd(l_phys,2)} m{standard_color}")
	...
	plt.plot(Ibias*1e3, Iac*1e3, label=f"{rd(l_phys,2)} m", color=cm(idx))
